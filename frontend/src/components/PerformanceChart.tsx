...
export function PerformanceChart({ indexId, guestBacktesting }: PerformanceChartProps) {
  ...
  const { data: backtestData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await authService.apiRequest(
        `${import.meta.env.VITE_API_URL || 'https://generatedassets1-8z68.onrender.com/api'}/indexes/backtest/${indexId}?period=${timeRange}`
      );
      if (!response.ok) {
  ...
  const generateNapkinChart = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'https://generatedassets1-8z68.onrender.com/api'}/napkin`, {
        method: 'POST',
        headers: {
