...
export default function Dashboard() {
  ...
  const { data: trendingIndexes = [], isLoading: trendingLoading } = useQuery({
    ...
    queryFn: async (): Promise<IndexData[]> => {
      const response = await fetch(
        "https://generatedassets1-8z68.onrender.com/api/trending-indexes"
      );
      if (!response.ok) {
  ...
  const { data: portfolio } = useQuery({
    ...
    queryFn: async (): Promise<PortfolioData> => {
      const response = await authService.apiRequest(
        "https://generatedassets1-8z68.onrender.com/api/portfolio"
      );
      if (!response.ok) {
  ...
  const { data: marketData } = useQuery({
    ...
    queryFn: async (): Promise<MarketData> => {
      const response = await fetch("https://generatedassets1-8z68.onrender.com/api/market-data");
      if (!response.ok) {
  ...
  const searchStocks = useCallback(async (query: string): Promise<void> => {
    ...
    try {
      const response = await fetch(
        `https://generatedassets1-8z68.onrender.com/api/search?query=${encodeURIComponent(query)}`
      );
      if (response.ok) {
  ...
  const addCustomStock = async (stock: StockSearchResult): Promise<void> => {
    try {
      ...
      const response = await fetch(
        `https://generatedassets1-8z68.onrender.com/api/stock-price/${stock.symbol}`
      );
